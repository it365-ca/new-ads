...
export const InterventionsSuiviMonthlyStats: React.FC<Props> = ({
  notes,
  enrollments,
  startYear,
  startMonth,
  ...
  const monthlyStats = useMemo(() => {
    ...
    // Créer un map des profils virtuels (prenom === "") à EXCLURE
    const virtualProfileIds = new Set(
      enrollments.filter(e => e.prenom === "" || !e.prenom).map(e => e._id)
    )
    
    // Calculer les stats par mois - Notes avec suivi uniquement, EXCLURE les profils virtuels
    const realNotes = (notes || []).filter(note => note.suivi === true && !virtualProfileIds.has(note.enrollmentId))
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-8">
      <h2 className="text-2xl font-bold text-gray-900 mb-6">✅ Interventions avec suivi</h2>
      
      <div className="overflow-x-auto">
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-gradient-to-r from-green-100 to-green-50">
              <th className="border border-gray-300 px-3 py-2 text-left font-semibold text-gray-700 sticky left-0 bg-green-100 z-10 text-xs">
                Type d'intervention
              </th>
              ...
              <th className="border border-gray-300 px-3 py-2 text-center font-bold text-green-700 bg-green-50 min-w-[70px] text-xs">
                Total
              </th>
            </tr>
          </thead>
          <tbody>
            {monthlyStats.interventionTypes.map((type, index) => {
              ...
              return (
                <tr key={type} className={`${bgColor} hover:bg-green-50 transition-colors`}>
                  ...
                  <td className="border border-gray-300 px-3 py-2 text-center font-bold text-green-700 bg-green-50 text-xs">
                    {rowTotal}
                  </td>
                </tr>
              )
            ...
            <tr className="bg-gradient-to-r from-green-100 to-green-50 font-bold">
              <td className="border border-gray-300 px-3 py-2 text-gray-800 sticky left-0 z-10 bg-green-100 text-xs">
                Total mensuel
              </td>
              {monthlyStats.months.map(month => {
                const columnTotal = calculateColumnTotal(month)
                return (
                  <td key={month} className="border border-gray-300 px-2 py-2 text-center text-green-700 text-xs">
                    {columnTotal}
                  </td>
                )
              })}
              <td className="border border-gray-300 px-3 py-2 text-center text-white bg-green-600 text-sm">
                {grandTotal}
              </td>
            </tr>
          </tbody>
